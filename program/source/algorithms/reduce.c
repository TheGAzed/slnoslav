...
state_t _check_magic_table(ulookup_t blocks, ulookup_t sum);
state_t _check_one_blocks(ulookup_t blocks, ulookup_t sum);
...
state_t _check_magic_table(ulookup_t blocks, ulookup_t sum) {
    ...
    for (ulookup_t i = 0; i < t.lengths[blocks - 2]; i++) {
...
void _reduce_row_multi_values(board_s board, state_array_s * current_state, ulookup_t index) {
    ...
    for (size_t i = 0; i < block; i++) {
        ...
        else push_stack(&multi, (ulookup_t)board.grid[row][col + i]);
    }
    
    while (!is_empty_stack(multi)) {
        ulookup_t i   = pop_stack(&multi);
...
void _reduce_col_multi_values(board_s board, state_array_s * current_state, ulookup_t index) {
    ...
    for (size_t i = 0; i < block; i++) {
        ...
        if (is_one_value(s)) { empty_blocks--; empty_sums -= state_to_sums(s); }
        else push_stack(&multi, (ulookup_t)board.grid[row + i][col]);
    }
