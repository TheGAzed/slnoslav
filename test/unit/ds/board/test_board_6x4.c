...
TEST game_grid_row_size(void) {
    ASSERT_EQ_FMTm(
        ...
        GAME_GRID_ROW_SIZE, b6x4.game.size[ROW],
        "%hhu"
    );
    PASS();
}
...
TEST game_grid_column_size(void) {
    ASSERT_EQ_FMTm(
        ...
        GAME_GRID_COL_SIZE, b6x4.game.size[COLUMN],
        "%hhu"
    );
    PASS();
}
...
TEST game_grid_count(void) {
    ASSERT_EQ_FMTm(
        ...
        GAME_GRID_COUNT, b6x4.game.count,
        "%hhu"
    );
    PASS();
}
...
TEST game_grid_empty_count(void) {
    ASSERT_EQ_FMTm(
        ...
        GAME_GRID_EMPTY_COUNT, b6x4.game.empty_count,
        "%hhu"
    );
    PASS();
}
    ...
    for (size_t i = 0; i < GAME_GRID_ROW_SIZE; i++) {
        for (size_t j = 0; j < GAME_GRID_COL_SIZE; j++) {
            ASSERT_EQ_FMTm(
                ...
                expected_grid[ROW][i][j], b6x4.game.grids[ROW][i][j],
                "%hhd"
            );
            ASSERT_EQ_FMTm(
                "GAME GRID COLUMN ELEMENTS DON'T MATCH",
                expected_grid[COLUMN][i][j], b6x4.game.grids[COLUMN][i][j],
                "%hhd"
            );
        }
    }
    ...
    for (size_t i = 0; i < GAME_GRID_ROW_SIZE; i++) {
        for (size_t j = 0; j < GAME_GRID_COL_SIZE; j++) {
            ASSERT_EQ_FMTm(
                ...
                expected_grid[i][j], b6x4.grid[i][j],
                "%hhd"
            );
        }
    }
...
TEST lookup_blocks(void) {
    ...
    for (size_t i = 0; i < GAME_GRID_EMPTY_COUNT; i++) {
        ASSERT_EQ_FMTm(
            ...
            expected_blocks[ROW][i], b6x4.blocks[ROW][i],
            "%hhd"
        );
        ASSERT_EQ_FMTm(
            "BOARD LOOKUP COLUMN BLOCKS DON'T MATCH",
            expected_blocks[COLUMN][i], b6x4.blocks[COLUMN][i],
            "%hhd"
        );
    }
    PASS();
    ...
    for (size_t i = 0; i < GAME_GRID_EMPTY_COUNT; i++) {
        ASSERT_EQ_FMTm(
            "BOARD LOOKUP ROW SUMS DON'T MATCH",
             expected_sums[ROW][i], b6x4.sums[ROW][i],
            "%hhd"
        );
        ASSERT_EQ_FMTm(
            "BOARD LOOKUP COLUMN SUMS DON'T MATCH",
            expected_sums[COLUMN][i], b6x4.sums[COLUMN][i],
            "%hhd"
        );
    }
    PASS();
...
TEST lookup_coordinates(void) {
    ...
    for (size_t i = 0; i < GAME_GRID_EMPTY_COUNT; i++) {
        ASSERT_EQ_FMTm(
            ...
            expected_coords[ROW][i], b6x4.coords[ROW][i],
            "%hhd"
        );
        ASSERT_EQ_FMTm(
            "BOARD LOOKUP COLUMN COORDINATES DON'T MATCH",
            expected_coords[COLUMN][i], b6x4.coords[COLUMN][i],
            "%hhd"
        );
    }
    PASS();
